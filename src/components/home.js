...
export default function Home() {
    ...
    useEffect( () => {
        axios.get('http://test.chakito.com/m/index.php/wp-json/wp/v2/pages?per_page=20&orderby=parent&parent_exclude=0&order=asc').then(res => {
    ...
    useEffect( ()=> {
        if(data.length === 0 && fetching === false) {
            navigate("/404",{state:{length:data.length,fetching:fetching}});
        }
    }, [])
